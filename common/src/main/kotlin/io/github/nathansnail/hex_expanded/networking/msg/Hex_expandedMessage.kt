...
sealed interface Hex_expandedMessageCompanion<T : Hex_expandedMessage> {
    ...
    fun apply(msg: T, supplier: Supplier<PacketContext>) {
        ...
        when (ctx.env) {
            EnvType.SERVER, null -> {
                Hex_expanded.LOGGER.debug("Server received packet from {}: {}", ctx.player.name.string, this)
                when (msg) {
                    is Hex_expandedMessageC2S -> msg.applyOnServer(ctx)
                    else -> Hex_expanded.LOGGER.warn("Message not handled on server: {}", msg::class)
                }
            }
            EnvType.CLIENT -> {
                ...
                when (msg) {
                    ...
                    else -> Hex_expanded.LOGGER.warn("Message not handled on client: {}", msg::class)
                }
            }
        }
    }
